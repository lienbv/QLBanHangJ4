<%@ page language="java" contentType="text/html; charset=utf-8"
	pageEncoding="utf-8"%>
<%@ taglib uri="http://java.sun.com/jstl/core_rt" prefix="c"%>
<%@ taglib uri="http://java.sun.com/jstl/fmt_rt" prefix="fmt"%>

<div
	class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
	<h1 class="h2">Quản lí Category</h1>
	<div class="btn-toolbar mb-2 ml-auto mb-md-0">
		<div class="btn-group mr-2">
			<a href="/ASMJ4/admin/Staff/create" type="button"
				class="btn btn-sm btn-outline-secondary">Thêm Category</a>
		</div>
	</div>
</div>
<div class="col-sm-12">

	<div class="row">
		<table class="table table-bordered mb-5 mt-3">
			<thead>
				<tr class="text-center">
					<th>Loại sản phẩm</th>
					<th>Edit</th>
					<th>Delete</th>
				</tr>

			</thead>
			<tbody>

				<c:forEach var="item" items="${list_category}">
					<tr>
						<td class="">${item.name}</td>
                        <td class=""><a href="/ASMJ4/admin/category/edit?id=${item.id }" type="button"
							class="btn btn-sm btn-outline-secondary"></a></td>
						<td><a onclick="showDelete(${item.id})" type="button"
							class="btn btn-sm btn-outline-secondary"><svg
									xmlns="http://www.w3.org/2000/svg" width="16" height="16"
									fill="currentColor" class="bi bi-trash-fill"
									viewBox="0 0 16 16">
                                    <path
										d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0z" />
                                </svg></a></td>
					</tr>

				</c:forEach>

			</tbody>
		</table>
	</div>

</div>

<script type="text/javascript">
	function showDelete(id) {
		var ques = confirm('Do you want to cancel?');
		if (ques == true) {

			window.location.href = '/ASMJ4/admin/category/delete?id='+ id;
		} else {

		}
	}
</script>


